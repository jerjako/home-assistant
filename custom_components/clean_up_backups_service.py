...
async def async_setup(hass, config):
    ...
    async def async_get_snapshots():
        ...
        try:
            with async_timeout.timeout(10, loop=hass.loop):
                resp = await session.get(hassio_url + 'snapshots', headers=headers)
            data = await resp.json()
            await session.close()
            return data['data']['snapshots']
        except aiohttp.ClientError:
            _LOGGER.error("Client error on calling get snapshots")
            await session.close()
        except asyncio.TimeoutError:
            _LOGGER.error("Client timeout error on get snapshots")
            await session.close()
        except Exception: 
            _LOGGER.error("Unknown exception thrown")
            await session.close()
    ...
    async def async_remove_snapshots(stale_snapshots):
        for snapshot in stale_snapshots:
            ...
            try:
                with async_timeout.timeout(10, loop=hass.loop):
                    resp = await session.post(hassio_url + 'snapshots/' + snapshot['slug'] + "/remove", headers=headers)
                res = await resp.json()
                ...
                _LOGGER.error("Client error on calling delete snapshot")
                await session.close()
            except asyncio.TimeoutError:
                _LOGGER.error("Client timeout error on delete snapshot")
                await session.close()
            except Exception: 
                _LOGGER.error("Unknown exception thrown on calling delete snapshot")
                await session.close()
